...
export class JourneyGraphComponent implements OnInit {
  ...
  constructor(private chRef: ChangeDetectorRef, private dialog: MatDialog, private elementRef: ElementRef) {}
  ...
  public addSchema(node: any, data: any): void {
    ...
    const dialogRef: MatDialogRef<AddJourneySchemaComponent> = this.dialog.open(
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      this.conditionData = JSON.parse(localStorage.getItem('condition')!);
      console.log(this.conditionData);
    
      this.delayData = JSON.parse(localStorage.getItem('delay')!);
      console.log(this.delayData);


      // const cData:any =localStorage.getItem('condition');
      // this.conditionData=JSON.parse(cData)
      // console.log(this.conditionData);

      // const dData:any=localStorage.getItem('delay');
      // const delayData=JSON.parse(dData);
  ...
  onNodeSelect(data: any) {
    ...
    const dialogConfig = new MatDialogConfig();
    dialogConfig.panelClass = 'center-dialog';
    const dialogRef: MatDialogRef<JourneySetupFormComponent> = this.dialog.open(
  ...
  addPath(parentId: any) {
    ...
    const dialogConfig = new MatDialogConfig();
    dialogConfig.panelClass = 'center-dialog';
    const dialogRef: MatDialogRef<AddPathDialogComponent> = this.dialog.open(
  ...
  removeChildren(node: any) {
    ...
    for (const childNode of childNodes) {
      ...
      const childIndex = this.nodes.findIndex(
        (n: any) => n.id === childNode.id
      );
      if (childIndex > -1) {
      ...
      this.links = this.links.filter(
        (link: any) =>
          link.source !== childNode.id && link.target !== childNode.id
      );
    }
  }
  ...
  ///Dispalying the localstorage data

  // actiondata(){
  //   const actiondata=JSON.parse(localStorage.getItem('formData')!)
